...
def crop_image(label, top, left, height, width, img_data):
    ...
    top_start = int(top - height_shift) if top - height_shift >= 0 else 0
    top_end = int(top + height_shift)
    left_start = int(left - width_shift) if left - width >= 0 else 0
    left_end = int(left + width_shift)
